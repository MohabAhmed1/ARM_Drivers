...
u8 MCP2515_U8ReadRegister(u8 address)
{
	...
	/**********************/
//	data2send=INST_READ;
//	SPI_u8SynchTransceive(SPI2, &data2send,0, 1);
//	data2send=address;
//	SPI_u8SynchTransceive(SPI2,&data2send,0, 1);
	/***********************/
	SPI_u8SynchTransceiveByte(SPI2, INST_READ);
	SPI_u8SynchTransceiveByte(SPI2, address);
	readValue=SPI_u8SynchTransceiveByte(SPI2, 0);
...
void MCP2515_VidWriteRegister(u8 address, u8 value)
{
	...
	SPI_u8SynchTransceiveByte(SPI2, INST_WRITE);
	SPI_u8SynchTransceiveByte(SPI2, address);
	SPI_u8SynchTransceiveByte(SPI2, value);
	SPI_voidChipSelect(0);
}
...
void MCP2515_VidReset()
{
	...
	SPI_u8SynchTransceiveByte(SPI2, INST_RESET);
	//SPI_u8SynchTransceive(SPI2, data, rex, 1);
...
void MCP2515_VidChangeBits(u8 RegAddress, u8 Mask, u8 Value)
{
	...
	//	SPI_U8MasterTransmit(Value);
	SPI_u8SynchTransceiveByte(SPI2, INST_BIT_MODIFY);
	SPI_u8SynchTransceiveByte(SPI2, RegAddress);
	SPI_u8SynchTransceiveByte(SPI2, Mask);
	SPI_u8SynchTransceiveByte(SPI2, Value);
	SPI_voidChipSelect(0);
}
...
{
	//	SPI_VidMasterInit();
	SPI_t spi ={.SPI_Number=SPI2,.SPI_Mode=SPI_Master,.SPI_Polartity=SPI_ClkPol_IdleHigh,
					.SPI_ClkRate=SPI_ClkRateDiv4,.SPI_DataOrder=SPI_LSBFirst,.SPI_Phase=SPI_ClkPhase_LeadingEdge};
	SPI_Init(&spi);
	MCP2515_VidReset();
...
void MCP2515_VidSendCANmsg(u8 bufIdx, u32 msgID, u8 * data,u8 properties)
{
	...
	SPI_u8SynchTransceiveByte(SPI2, INST_WRITE);
	SPI_u8SynchTransceiveByte(SPI2, TXB0CTRL);

	/* Setup message priority */
	//SPI_U8MasterTransmit(properties >> 6);
	SPI_u8SynchTransceiveByte(SPI2, properties >> 6);
	...
	//		SPI_U8MasterTransmit(0);
	SPI_u8SynchTransceiveByte(SPI2,(u8)(msgID>>3));
	SPI_u8SynchTransceiveByte(SPI2,(u8)(msgID<<5));
	SPI_u8SynchTransceiveByte(SPI2, 0);
	SPI_u8SynchTransceiveByte(SPI2, 0);
...
//	SPI_u8SynchTransceive(SPI2,&data2send,0,8);
/******************************************/
	SPI_u8SynchTransceiveByte(SPI2, properties & 0x0F);

	u8 rx[8];
	/* Store the message into the buffer */
		for(u8 i = 0; i < (properties & 0x0F); i++)
		{
			SPI_u8SynchTransceiveByte(SPI2, data[i]);
		}
			//SPI_u8SynchTransceive(SPI2,data,rx,8);
	...
	SPI_u8SynchTransceiveByte(SPI2, INST_RTS_TXB0);
	SPI_voidChipSelect(0);
}
...
void MCP2515_VidSetMask(u8 maskAddress, u32 maskValue, u8 extended)
{
	...
	SPI_u8SynchTransceiveByte(SPI2, INST_WRITE);
	SPI_u8SynchTransceiveByte(SPI2, maskAddress);
	...
	if(extended)
	{
		...
		SPI_u8SynchTransceiveByte(SPI2, (u8) (maskValue >> 3));
		SPI_u8SynchTransceiveByte(SPI2, (u8) (maskValue << 5) | (1<<3) | (u8) (maskValue >> 27));
		SPI_u8SynchTransceiveByte(SPI2, (u8) (maskValue >> 19));
		SPI_u8SynchTransceiveByte(SPI2, (u8) (maskValue >> 11));
		/***************************************/
...
//		SPI_u8SynchTransceive(SPI2, &data2send, 0, 1);
	...
	else
	{
		...
		SPI_u8SynchTransceiveByte(SPI2, (u8) (maskValue >> 3));
		SPI_u8SynchTransceiveByte(SPI2, (u8) (maskValue << 5));
	}
	SPI_voidChipSelect(0);
}
...
void MCP2515_VidReceiveCANmsg(void)
{
	...
	SPI_u8SynchTransceiveByte(SPI2, INST_READ);
	SPI_u8SynchTransceiveByte(SPI2, RXB0CTRL);


//	for(u8 i=0; i<14; i++)
		//		readBuffer[i] = SPI_U8MasterTransmit(0);
		SPI_u8SynchTransceive(SPI2, 0,readBuffer, 14);
